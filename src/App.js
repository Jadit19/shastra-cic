...
const App = () => {
  ...
  const handleChange = event => {
    ...
    if (name === 'sma') {
      ...
      const eaRad =
        2 *
        Math.atan2(
          Math.sqrt(1 - e) * Math.sin(taRad / 2),
          Math.sqrt(1 + e) * Math.cos(taRad / 2)
        )
      ...
      const maRad =
        taRad -
        2 * e * Math.sin(taRad) +
        (0.75 * e * e + 0.125 * e * e * e * e) * Math.sin(2 * taRad) -
        (1 / 3) * e * e * e * Math.sin(3 * taRad) +
        (5 / 32) * e * e * e * e * Math.sin(4 * taRad)
      setParams({
        ...params,
        trueAnomaly: value,
      ...
      const taRad =
        maRad +
        (2 * e - 0.25 * e * e * e) * Math.sin(maRad) +
        1.25 * e * e * Math.sin(2 * maRad) +
        (13 / 12) * e * e * e * Math.sin(3 * maRad)
  ...
  return (
    <>
      <h1>Satellite Orbit Simulator</h1>
      <div className='app'>
        <div className='container'>
          <form onReset={handleReset} onSubmit={handleSubmit}>
            ...
            {params.eccentricity === 0 ? null : (
              <div className='form__field'>
                <TextField
                  style={inputStyle}
                  type='number'
                  variant='outlined'
                  label='Argument of Periapsis (in degrees)'
                  name='periapsis'
                  required
                  value={params.periapsis}
                  onChange={handleChange}
                />
              </div>
            )}
            <div className='form__field'>
              <div className='quat'>
                <TextField
                  ...
                  type='number'
                  variant='outlined'
                  label='True Anomaly (in degrees)'
                  name='trueAnomaly'
                  required
                  value={params.trueAnomaly}
                  onChange={handleChange}
                />
                <TextField
                  style={{
            ...
            <FormControlLabel
              ...
              control={
                <Checkbox
                  value={acceptQuaternions}
                  onClick={() => {
                    setAcceptQuaternions(!acceptQuaternions)
                  }}
                />
              }
              label='Input Camera Orientation'
            />
            ...
            <div className='form__field'>
              <FormControlLabel
                ...
                control={
                  <Checkbox
                    value={plotTrajectory}
                    onClick={() => {
                      setPlotTrajectory(!plotTrajectory)
                    }}
                  />
                }
                ...
                <TableBody>
                  <TableRow>
                    ...
                    <TableCell>
                      {coordinates === null
                        ? 'NA'
                        : coordinates.satellite.longitude}
                    </TableCell>
                  </TableRow>
                  <TableRow>
                    ...
                    <TableCell>
                      {coordinates === null
                        ? 'NA'
                        : coordinates.satellite.latitude}
                    </TableCell>
                  </TableRow>
                  <TableRow>
                  ...
                  <TableRow>
                    ...
                    <TableCell>
                      {coordinates === null
                        ? 'NA'
                        : coordinates.image.longitude}
                    </TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell>Imaging Latitude</TableCell>
                    <TableCell>
                      {coordinates === null ? 'NA' : coordinates.image.latitude}
                    </TableCell>
                  </TableRow>
                </TableBody>
              </Table>
            ...
            <ComposableMap projectionConfig={{ scale: 140 }}>
              ...
              {plotTrajectory === false
                ...
                : trajectory.map((traj, idx) => (
                    <Marker
                      coordinates={[traj.longitude, traj.latitude]}
                      key={idx}
                    >
              ...
              {coordinates === null ? null : (
                <>
                  ...
                  {coordinates.image.longitude === NaN ||
                  coordinates.image.latitude === NaN ? null : (
                    <Marker
                      coordinates={[
                        coordinates.image.longitude,
                        coordinates.image.latitude
                      ]}
                    >
                      <circle r={7} fill='red' />
                    </Marker>
